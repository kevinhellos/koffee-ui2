...
export default function Sidebar({ index, name, href, js, experimental} : 
  ...
  return (
    <li className={`px-3 py-2 ${pathname === "/components/"+ name.toLocaleLowerCase() ? 
    "bg-slate-50 rounded-sm font-medium border-l-2 border-black" : ""} `} key={index}>
      <Link className="w-full kf-link" href={href}>
        {name}
      </Link>
      {js && <span className="border text-xs px-3 py-1 rounded-full ms-2">JS</span>}
      {experimental && 
        <span className="text-xs px-1 py-1 rounded-full ms-2 kf-bg-purple float-end w-fit">
